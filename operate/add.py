...
class Add:
    ...
    def __init__(self, master):
        ...
        self.frame4 = Frame(self.window, width=900, height=550, bg='#F0F0F0')
        self.frame4.pack()
        self.add_window()
    ...
    def add_window(self):
        """添加窗口"""
        # self.window = master
        # self.frame = Frame(self.frame4, width=800, height=600, bg='#F0F0F0')
        # self.frame.pack()
        # 创建标签,显示文字标题
        Label(self.frame4, text='添加学生信息', font=('隶书', 20),
        ...
        entry = [0] * 7
        for i in range(7):
            Label(self.frame4, text=add_title[i], font=('隶书', 15),
                  justify=CENTER).place(x=260, y=100 + i * 50, width=100, height=30)
            # 创建性别RadioButton单选按钮
            if i == 2:
                Radiobutton(self.frame4, text='男', variable=self.get[i], value='男',
                            font=('隶书', 15), justify=CENTER).place(x=350, y=100 + i * 50, width=50, height=30)
                Radiobutton(self.frame4, text='女', variable=self.get[i], value='女',
            ...
            # 创建年龄Spinbox控件，指定输入范围
                ...
                Spinbox(self.frame4, from_=17, to=24, width=10, font=('隶书', 16), state='readonly', justify=CENTER,
            ...
            # 创建学院Combobox下拉列表
                ...
                self.college_combobox = Combobox(self.frame4, state='readonly', textvariable=self.get[i],
            ...
            # 创建专业Combobox下拉列表,与学院相关联
                ...
                self.major_combobox = Combobox(self.frame4, textvariable=self.get[i], state='readonly',
                                               font=('宋体', 12), justify=CENTER)
            ...
            # 创建其他Entry控件,接受文本
                ...
                entry[i] = Entry(self.frame4, textvariable=self.get[i], font=('宋体', 15),
                                 justify=CENTER)
        ...
        # 添加按钮
        self.btn_add = Button(self.frame4, text='添加', width=15, height=1, cursor='hand2', font=('黑体', 14), )
        # command=self.add_click)
        self.btn_add.place(x=300, y=450, width=200, height=40)
        self.btn_add.bind('<Button-1>', self.add_click)
        # 匿名函数,重置输入框
        self.btn_reset = Button(self.frame4, text='重置', width=15, height=1, cursor='hand2', font=('黑体', 14),
